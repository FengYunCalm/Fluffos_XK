...
- **现代 LPC，按需启用**：`#pragma modern_lpc` 与 `#pragma strict_owner`
  在旧 LPC 之上提供 owner-aware 审计、owner-safe async/future API、snapshot/value
  object 和 VM 热路径诊断，不破坏旧 mudlib。
- **编码边界可控**：VM 内部字符串继续使用规范 UTF-8；源码、玩家连接、gateway payload
  和外部数据可在边界显式使用 GBK、GB2312、Big5 等编码，并通过 ICU 转码进入 VM。
...
FluffOS_XK 不把 LPC 替换成其他脚本语言，而是在 LPC 之上增加可选的现代 profile：

- `#pragma modern_lpc` 启用现代诊断和 owner-safe runtime API；
- `#pragma strict_owner` 把 owner 迁移问题固定成新代码的严格审计结果；
- `lpcc --owner-audit --format=json` 输出源码编码、profile pragma、cross-owner
  write 风险、可变 callback payload、热路径直接持久化和修复建议；
- `freeze`、`snapshot`、`owner_async`、`owner_await`、`owner_commit` 和
  `owner_snapshot_persist` 提供明确的 payload、future、commit 和持久化边界；
- VM profiling 与 benchmark 暴露 opcode、efun、`call_other`、function pointer、
  parser/action、mapping 和 string 热路径。

旧 LPC 默认行为不变，只有 mudlib 显式启用 profile 后才进入这些新约束。遗留中文源码或
客户端可以在边界使用 `#pragma source_encoding("GBK")`、`set_encoding("GBK")`、
`string_encode`、`string_decode` 和 `buffer_transcode`，不需要把 VM 内部字符串语义改成
GBK。
