...
TEST_F(DriverTest, TestVmOwnerExecutorCommandFrameRestoreDispatchesWithoutLpc) {
  ...
  ASSERT_STREQ(mapping_string(vm_context_contract, "ordinary_lpc_next_blocker"), "object_refs_owner_local");
  free_mapping(running);
...
TEST_F(DriverTest, TestVmOwnerRuntimeReportsExecutorTaskContract) {
  ...
  auto assert_contract = [&](mapping_t* status) {
    ...
    ASSERT_STREQ(mapping_string(vm_context_contract, "ordinary_lpc_blocker"),
                 "object_refs_and_object_store_not_owner_local");
    ...
    ASSERT_EQ(mapping_number(vm_context_contract, "ordinary_lpc_satisfied_gate_count"), 9);
    ASSERT_EQ(mapping_number(vm_context_contract, "ordinary_lpc_blocked_gate_count"), 2);
    ...
    ASSERT_EQ(mapping_number(gate_entry("object_refs_owner_local"), "satisfied"), 0);
    ASSERT_STREQ(mapping_string(gate_entry("object_refs_owner_local"), "blocker"),
                 "bare_object_pointer_cross_owner_refs");
    ...
    ASSERT_EQ(command_executor_gates->size, 5);
    ...
    ASSERT_STREQ(mapping_string(gateway_contract, "next_blocker_chain"),
                 "ordinary_lpc_ready/object_refs_owner_local");
    ...
    ASSERT_EQ(dispatch_contracts->size, 10);
    ...
    auto* ordinary_lpc = mapping_entry(contract, "lpc");
    ASSERT_STREQ(mapping_string(ordinary_lpc, "executor_mode"), "rejected");
    ASSERT_EQ(mapping_number(ordinary_lpc, "executor_safe"), 0);
    ASSERT_EQ(mapping_number(ordinary_lpc, "main_required"), 0);
    ASSERT_EQ(mapping_number(ordinary_lpc, "rejected"), 1);
  };
...
TEST_F(DriverTest, TestVmOwnerThreadRejectsUnregisteredLpcTask) {
  ...
  ASSERT_STREQ(mapping_string(submitted, "contract_reason"), "ordinary LPC is default closed");
...
TEST_F(DriverTest, TestVmOwnerThreadRejectsRegisteredDomainLpcTasks) {
  ...
  for (const auto* method : methods) {
    ...
    ASSERT_STREQ(mapping_string(submitted, "contract_reason"), "ordinary LPC is default closed") << method;
