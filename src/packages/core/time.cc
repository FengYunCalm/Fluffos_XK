...
#include <iomanip>
#include <sstream>
#include <vector>  // #1247 TIME-1: strftime heap buffer
...
void f_strftime() {
  ...
  // #1247 TIME-1: heap, not a stack VLA: __MAX_STRING_LENGTH__ defaults to
  // 1MB and is admin-configurable up to INT_MAX, so `char buf[max_string_length]`
  // risked a stack overflow on every call.
