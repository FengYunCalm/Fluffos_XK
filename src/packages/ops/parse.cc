...
static svalue_t *living_parse(array_t *obarr, array_t *warr, int *cix_in, int *fail) {
  ...
  *fail = 0;
  ...
  for (obix = 0; obix < obarr->size; obix++) {
    // #1247 PARSE-1: the array may hold non-objects (a caller-supplied 0, or a
    // destructed object that check_for_destr turned into the number 0); skip
    // them instead of dereferencing u.ob.
