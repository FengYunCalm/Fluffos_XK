...
// ---------------------------------------------------------------------------
// E3 recompile_object() internals (v0.4 §8). Building blocks only; the efun
// wrapper and quiescence transaction arrive in P4/P5.
// ---------------------------------------------------------------------------
...
StagedProgram compile_program_for_recompile(object_t *blueprint) {
  ...
  // Compile the blueprint's own source file (obname without the .c suffix is
  // the load name; the source path resolves the same way load_object does).
  ...
  // Mirrors load_object's path resolution (legal_path + ".c").
...
namespace {
// Depth-first walk of the variable block: inherits first (in order), then
// this program's own definitions -- the actual slot layout.
void walk_layout(program_t *prog, int *next_slot, RecompileLayout *out, std::string *digest) {
  for (int i = 0; i < prog->num_inherited; i++) {
    ...
    // type_mod is a compile-time concept; the driver-side proxy is the
    // inherited variable count contributed by this inherit.
...
// E3 P4: frozen target snapshot (v0.4 §8.3).
void snapshot_recompile_targets(object_t *blueprint, RecompilePrepared *prepared) {
  ...
  for (object_t *ob = obj_list; ob; ob = ob->next_all) {
    ...
    // Precompute program-derived flags from the NEW program.
    uint32_t derived = 0;
...
// E3 P4: no-fail commit (v0.4 §9.2). Every allocation/LPC/error path must
// have completed before this runs; the caller holds the owner runtime FROZEN.
void commit_recompile_targets_noexcept(RecompilePrepared *prepared) noexcept {
  program_t *new_prog = prepared->staged.prog;
  if (!new_prog) return;
  // Pin the staging program once so the swap loop cannot free it midway.
  new_prog->ref++;

  constexpr uint32_t kProgramDerivedFlags = O_WILL_CLEAN_UP;
  // v0.4 §9.1: every target now holds a reference to new_prog (the objects
  // free their program reference on dealloc). Accounted before any swap so
  // the program cannot die mid-commit even if the staging pin is dropped.
  ...
  // Release the N old-program references held by the targets (each object
  // held one). The transaction pin keeps old_prog alive until this loop.
  for (size_t i = 0; i < prepared->targets.size(); i++) {
    program_t *old_ref = prepared->old_prog;
    free_prog(&old_ref);
  }

  // Drop the staging pin; the targets now hold the new program's refs.
  program_t *staged_pin = new_prog;
  free_prog(&staged_pin);

  // The prepared struct still owns one staging ref (its destructor frees
  // it). The snapshot add_refs on the targets are released by the caller
  // AFTER this function returns (still under FROZEN but outside the
  // no-fail segment), and the target ob pointers stay valid until then.
}
