...
- `current_object`/`current_prog`/`previous_ob`/`caller_type` execution frame setter，`command_giver` 的 VMContext setter/保存栈同步，以及 `current_interactive` 的 VMContext setter/scope，用于收敛解释器执行状态的保存恢复。
...
- VMExecutionScope、execution frame setter、VMCurrentInteractiveScope、command_giver 保存栈和 VMOwnerScope 的保存恢复。
...
- apply/direct/function pointer 高频入口已通过 VMContext API 设置 `current_object`、`current_prog`、`previous_ob` 和 `caller_type`；interactive/gateway 高频入口已通过 VMContext API 设置 `current_interactive`，`command_giver` 保存栈会同步 VMContext，`this_player()` 读取 VMContext execution 快照；但 eval stack 和对象存储还没有完全 owner-local 化。
