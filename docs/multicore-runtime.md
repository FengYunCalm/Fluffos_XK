...
- 不支持任意 LPC 在后台线程自由执行。
- gateway/player input、async/db/file completion、DNS callback、socket read/write/close callback 仍以主线程 owner queue 作为兼容入口；heartbeat 和 callout 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；worker v2 `compute_result` 可由 owner executor 消费，但 async/DNS/socket/gateway/input 真实 LPC 回调尚未变成后台 owner executor 并行执行。
