...
- XiaKeXing `multicore mode : enforced` 当前只验证到启动和登录；mixed 命令会因 server 侧 `owner_gameplay_d.c::query_actor_id()` 同步跨 owner `call_other` 被 runtime 拦截而超时，说明业务命令链路还未完成 message/future 或 snapshot 迁移。
...
- enforced 模式已经能暴露真实 mudlib 迁移缺口，但不能视为 XiaKeXing 完整可运行模式；目前核心玩家命令仍会命中同步跨 owner 调用拦截。
