...
- production gate 状态合同：`gateway_owner_task_contract` 与 `owner_executor_boundary_contract` 暴露 `mudlib_audit_required=1`、`mudlib_cross_owner_hotspots_ready=0`、`mudlib_cross_owner_hotspots_blocker=real_mudlib_audit_not_complete`、`production_gate_ready=0`、`production_gate_blocker=real_mudlib_final_audit_not_complete`、`production_gate_required_users=1,3,10`、`production_gate_required_durations=smoke,30m`、`production_gate_pressure_evidence_ready=1` 和 `production_gate_pressure_evidence=xkx_audit_10_users_30m_2026_06_25_zero_timeouts_zero_gateway_errors`。这些字段只约束真实 mudlib 验收状态，说明当前压力口径已由 10 用户 30 分钟 audit 复核满足；生产 ready 仍取决于 final audit 零 blocker，不改变 owner executor 调度路径。
