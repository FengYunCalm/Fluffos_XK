...
- `current_interactive` 的 VMContext setter/scope，用于收敛 interactive/gateway 执行状态的保存恢复。
...
- VMExecutionScope、VMCurrentInteractiveScope 和 VMOwnerScope 的保存恢复。
...
- interactive/gateway 高频入口已通过 VMContext API 设置 `current_interactive`，但 `this_player()`、`command_giver` 和更深层 apply 状态还没有完全 owner-local 化。
