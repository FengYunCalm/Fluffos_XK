...
- 主线程 owner queue，仍用于按 owner/epoch 投递和分发 gateway/player input 以及 main-required cleanup/reply；heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 在 `audit`/`enforced` 且 owner thread 可用时优先投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；`socket_release` 因同步 release/acquire 语义仍保留主线程路径。
...
主线程 owner queue 当前承担过渡职责：网络输入、gateway 命令以及 executor callback cleanup/reply 仍进入 per-owner 队列并在主线程绑定 `VMOwnerScope` 后执行；heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 在 `audit`/`enforced` 且 owner thread 可用时优先进入 owner executor，执行前校验 owner epoch，stale/destructed 会 drop 并记录 trace，LPC 引用释放仍通过主线程 cleanup 完成；`socket_release` 因同步 release/acquire 语义仍保留主线程路径。这样能先固定 owner 调度语义和 stale epoch 防线，同时避免在全局 VM 状态还未完全迁出前贸然让后台线程执行任意 LPC。
...
- 不支持任意 LPC 在后台线程自由执行。
- gateway/player input 仍以主线程 owner queue 作为兼容入口；heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；worker v2 `compute_result` 可由 owner executor 消费，但 gateway/input 真实玩家命令 LPC 回调尚未变成后台 owner executor 并行执行；`socket_release` 仍是 main-required 例外。
