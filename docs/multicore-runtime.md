...
- production gate 状态合同：`gateway_owner_task_contract` 与 `owner_executor_boundary_contract` 暴露 `mudlib_audit_required=1`、`mudlib_cross_owner_hotspots_ready=0`、`mudlib_cross_owner_hotspots_blocker=real_mudlib_audit_not_complete`、`production_gate_ready=0`、`production_gate_blocker=real_mudlib_pressure_not_verified`、`production_gate_required_users=1,3,10,50,100` 和 `production_gate_required_durations=smoke,30m,2h,overnight`。这些字段只约束真实 mudlib 验收状态，防止把短 smoke 误读为生产完成，不改变 owner executor 调度路径。
