...
- runtime/thread status 的 `gateway_owner_task_contract` 暴露阶段 8 input/gateway 的当前执行边界：`input_model=owner_executor_with_main_fallback`、`executor_migration_state=owner_executor_active`、`ordinary_lpc_ready_required=0`，并把 `process_user_command` 固定为 `executor_safe_callback` / `owner_executor` 合同，`gateway_receive` 仍经 `owner_main_queue`，`gateway_logon`/`gateway_disconnected` 仍是 `direct_main_owner_scope`。`process_user_command` 在 `audit`/`enforced` 且 owner thread 可用时投递 `gateway_command_execute` executor callback，`off` 或 executor unavailable 时回退 owner main queue。`process_user_command` 合同额外声明 `payload_key=gateway_command_input`、`input_payload_policy=buffer_metadata_no_raw_command_text`、`command_input_source=interactive_text_buffer`、`command_text_snapshot_policy=owner_private_redacted_from_trace`、`command_text_snapshot_ready=1`、`command_input_callback_state_policy=redacted_input_to_get_char_state_v1`、`command_input_callback_snapshot_ready=1`、`command_input_callback_frame_model=owner_command_frame_input_callback_detach_v1`、`command_input_callback_frame_detach_ready=1`、`command_input_callback_frame_executor_ready=1`、`command_executor_blocker=""`、`command_consume_model=owner_owned_snapshot_owner_executor_consume`、`command_consume_snapshot_ready=1`、`command_consume_executor_ready=1`、`command_consume_blocker=""`、`command_reply_queue_model=main_reply_queue_after_owner_command`、`command_reply_queue_task_type=command_reply`、`command_reply_queue_task_key=prompt_telnet_reschedule_io`、`command_reply_queue_ready=1`、`command_reply_queue_main_required=1`、`command_mode_delta_model=owner_command_frame_mode_delta`、`command_mode_delta_localecho_restore_boundary=main_reply_queue_after_command_consume`、`command_mode_delta_localecho_restore_ready=1`、`command_mode_delta_terminal_mode_task_type=command_mode_delta`、`command_mode_delta_terminal_mode_task_keys=get_char_linemode_restore,single_char_escape_linemode,single_char_escape_charmode_restore`、`command_mode_delta_terminal_mode_boundary=main_mode_delta_queue_after_command_consume`、`command_mode_delta_terminal_mode_ready=1`、`command_mode_delta_ready=1`、`requires_target_handle=1`、`requires_frozen_payload=1`、`execution_frame_model=gateway_command_execution_frame_v1`、`execution_frame_policy=owner_scope_current_interactive_command_giver`、`command_execution_frame_restore_policy=owner_executor_vmcontext_restore`、`command_execution_frame_restore_ready=1`、`command_execution_frame_restore_blocker=\"\"` 和 `execution_frame_executor_ready=1`；合同还声明 `command_stale_guard=owner_epoch_target_handle_guard`、`command_stale_trace_state=thread_executor_callback_stale`、`command_stale_target_status=owner_epoch_mismatch`。实际 gateway command executor trace 会携带 `gateway_command_execute/process_user_command` 的 `executor_callback_queued`、`thread_executor_callback_dispatched`、`snapshot_dispatched` 和 `thread_executor_callback_completed` 状态；executor callback 使用 interactive text buffer 的 owner-private 命令快照调用 `process_user_command_snapshot()`、execution-frame 捕获元数据以及 `execution_frame_restore_policy=owner_executor_vmcontext_restore` / `execution_frame_restore_ready=1` / `execution_frame_restore_blocker=\"\"`，但不会把原始命令文本写入 trace；如果入队后 owner epoch 漂移，owner executor 会在 callback guard 阶段以 `thread_executor_callback_stale` 丢弃旧 task，并通过 ObjectHandle 报告 `owner_epoch_mismatch`，不会消费旧命令。四类入口都要求恢复 owner scope、`current_interactive` 和 `command_giver`，这证明 gateway 玩家命令已在受控 owner executor callback 中执行，且仍保留 owner main queue fallback；下一阻塞点被拆成 `command_executor_readiness_gates` 机器可读门禁，`command_executor_readiness_gate_model=all_gates_required_before_owner_executor`、gate count 为 7，其中 `owner_epoch_target_handle_guard`、`owner_owned_command_snapshot`、`owner_owned_command_consume`、`owner_executor_command_consume_entry`、`owner_executor_frame_restore` 和 `ordinary_lpc_ready` 均已满足；7 项 command executor readiness gate 已全部满足，`command_executor_next_gate=mudlib_cross_owner_hotspots`、`command_executor_next_blocker=""`；`gateway_command_executor_activation` 已进入 guarded executor-safe 合同。普通 LPC 显式开放路径仍不等于任意 legacy LPC 默认开放；gateway command executor 只通过 `gateway_command_execute` driver callback allowlist 执行玩家命令。`executor_task_dispatch_contracts` 另有 `gateway_command` / `gateway_command_executor_activation` descriptor，当前 `executor_mode=executor_safe`、`executor_safe=1`、`rejected=0`，owner 线程仍保留 `gateway_command` / `thread_gateway_command_executor_guarded` activation descriptor；真实玩家命令执行走 `gateway_command_execute` / `process_user_command` executor callback。`command_side_effect_readiness_gates` 进一步把 activation blocker 拆成 5 项：`interactive_buffer_consume` 和 `prompt_telnet_reschedule_io` 已满足；`input_to_get_char_state`、`process_input_add_action_parser`、`interactive_mode_flags` 也已收敛进 owner command frame，blocked gate 为 0。每个 side-effect gate 同时暴露 `state_owner`、`migration_boundary`、`side_effect_class`、`snapshot_policy`、`snapshot_ready`、`state_redacted` 和 `blocks_activation`，用于把后续迁移边界固定为机器可读合同；顶层合同同步暴露 `command_side_effect_satisfied_gate_count=5`、`command_side_effect_blocked_gate_count=0`、`command_side_effect_snapshot_gate_count=5`、`command_side_effect_snapshot_ready_count=5`、`command_side_effect_observability_ready=1` 与 `command_side_effect_activation_ready=1`，`gateway_command_executor_activation` 嵌套合同也暴露同一组 side-effect readiness 和 `activation_blocker=""`，明确 side-effect activation、guarded gateway activation 和 `gateway_command_execute` 真实入口均已齐备；生产开放仍需要 mudlib audit 和长压测。gateway command payload 还暴露 `input_callback_state_policy=redacted_input_to_get_char_state_v1`、`input_callback_state_snapshot_ready=1`、`input_callback_frame_model=owner_command_frame_input_callback_detach_v1`、`input_callback_frame_detach_ready=1`、`input_callback_frame_executor_ready=1`、`input_callback_active`、`input_callback_single_char`、`input_callback_noescape`、`input_callback_noecho`、`input_callback_carryover_count` 以及函数/对象 redacted 标志；当前 C++ 回归已覆盖 inactive input callback、active `input_to`、active `get_char`/single-char 三类 payload 快照，并验证 `interactive_input_callback/*/frame_detached` trace，且不暴露 callback function、callback object 或 raw command text；这只是 input_to/get_char frame detach 子边界和脱敏观测，`input_to_get_char_state` gate 已满足，但不表示 input callback 已可后台执行。gateway command payload 同时暴露 `process_input_add_action_parser_state_policy=redacted_process_input_add_action_parser_state_v1`、`process_input_add_action_parser_has_process_input`、`process_input_add_action_parser_safe_parse_fallback`、`process_input_add_action_parser_requires_command_giver` 以及 command_giver/command_text redacted 标志；C++ 回归已验证这些字段存在且不暴露 command_giver 对象或命令文本；这只是 process_input/add_action/parser 状态的脱敏观测，`process_input_add_action_parser` gate 已满足，但不表示普通 legacy LPC 默认开放。gateway command payload 还暴露 `interactive_mode_flags_state_policy=redacted_interactive_mode_flags_v1`、`interactive_mode_flags_state_snapshot_ready=1`、`interactive_mode_flags_state_redacted=1`、`interactive_mode_terminal_mode_delta_boundary=main_mode_delta_queue_after_command_consume`、`interactive_mode_terminal_mode_delta_ready=1`、`interactive_mode_terminal_linemode_restore_required` 和 `interactive_mode_terminal_charmode_restore_required`，以及 noecho/noescape/single_char/was_single_char/MXP/ed buffer 状态快照；C++ 回归已验证 baseline、active `input_to`、active `get_char`/single-char 快照，以及 `command_mode_delta/get_char_linemode_restore` 同 owner main task queued/dispatched trace，且仍不暴露 raw command text；这只是 terminal mode delta 子边界和 interactive mode flags 的脱敏观测，`interactive_mode_flags` gate 已满足，但不表示普通 legacy LPC 默认开放。gateway command payload 还暴露 `prompt_telnet_reschedule_state_policy=redacted_prompt_telnet_reschedule_io_v1`、`prompt_telnet_reschedule_state_snapshot_ready=1`、`prompt_telnet_reschedule_state_redacted=1`、`prompt_telnet_reschedule_boundary=main_reply_queue_after_owner_command`、`prompt_telnet_reschedule_reply_queue_ready=1`、`prompt_telnet_reschedule_blocks_activation=0`、`prompt_has_write_prompt`、`prompt_text_redacted`、`prompt_write_prompt_apply_required`、`telnet_handle_active`、`telnet_using_telnet`、`telnet_suppress_ga`、`telnet_ga_required` 和 `reschedule_cmd_in_buf`；C++ 回归已验证默认 gateway 会话的 prompt/telnet/reschedule 快照、同 owner `command_reply/prompt_telnet_reschedule_io` main task queued/dispatched trace，且不暴露 prompt text 或 raw command text；这表示 prompt/telnet/reschedule I/O 已迁到命令后的主线程 reply queue，`prompt_telnet_reschedule_io` gate 已满足，但该 reply queue 仍是 main-required 网络 I/O 边界，不表示网络 reply I/O 可后台执行。
- production gate 状态合同：`gateway_owner_task_contract` 与 `owner_executor_boundary_contract` 暴露 `mudlib_audit_required=1`、`mudlib_cross_owner_hotspots_ready=1`、`mudlib_cross_owner_hotspots_blocker=""`、`mudlib_cross_owner_hotspots_evidence=xkx_5513c8a12_multicore_mudlib_audit_2026_06_25_zero_delayed_object_payloads`、`production_gate_ready=0`、`production_gate_blocker=socket_release_owner_safe_handshake_not_ready`、`production_gate_required_users=1,3,10`、`production_gate_required_durations=smoke,30m`、`production_gate_pressure_evidence_ready=1` 和 `production_gate_pressure_evidence=xkx_audit_10_users_30m_2026_06_25_zero_timeouts_zero_gateway_errors`。这些字段只约束真实 mudlib 验收状态，说明当前压力口径已由 10 用户 30 分钟 audit 复核满足，真实 XiaKeXing mudlib final audit 已收口；生产 ready 仍取决于 `socket_release` owner-safe handshake，不改变 owner executor 调度路径。
- process_input/add_action parser frame 增量：gateway contract 与 gateway command payload 暴露 `process_input_add_action_parser_frame_model=owner_command_parser_context_v1`、`process_input_add_action_parser_frame_ready=1`、`process_input_add_action_parser_frame_executor_ready=1`、`process_input_add_action_parser_blocker=""`；`process_input()` 的 parse fallback/override 路径统一记录 `interactive_command_parser/safe_parse_command/frame_entered` trace。该边界只说明 parser command_giver context 已显式 owner-frame 化，`process_input_add_action_parser` gate 已满足，但不能据此开放普通 legacy LPC。
- process_input apply frame 增量：gateway contract 与 gateway command payload 暴露 `process_input_apply_frame_model=owner_command_frame_process_input_apply`、`process_input_apply_frame_task_type=interactive_command_parser`、`process_input_apply_frame_ready=1`、`process_input_apply_frame_executor_ready=1`；触发路径记录 `interactive_command_parser/process_input_apply/frame_entered` trace。该边界说明 `APPLY_PROCESS_INPUT` 调用已显式 frame 化并可随 gateway command executor 运行，`process_input_add_action_parser` gate 已满足，但不能据此开放普通 legacy LPC。
- input_to/get_char mode delta 增量：gateway contract 与 gateway command payload 暴露 `command_input_callback_mode_delta_model=owner_command_frame_input_callback_mode_delta`、`command_input_callback_mode_delta_ready=1`、`command_input_callback_mode_delta_executor_ready=1` 和 payload 侧 `input_callback_mode_delta_*`；`call_function_interactive()` 的 mode flag 清理统一记录 `interactive_input_callback_mode/input_to_get_char_mode_flags/frame_detached` trace。该边界只说明 `NOESC`、`SINGLE_CHAR`、`NOECHO` 清理已显式 frame 化，`input_to_get_char_state` gate 已满足，但不能据此执行后台 input callback。
- input_to/get_char callback apply 增量：gateway contract 与 gateway command payload 暴露 `command_input_callback_apply_frame_model=owner_command_frame_input_callback_apply`、`command_input_callback_apply_frame_task_type=interactive_input_callback`、`command_input_callback_apply_frame_ready=1`、`command_input_callback_apply_frame_executor_ready=1` 和 payload 侧 `input_callback_apply_frame_*`；`call_function_interactive()` 的回调 apply/function-pointer dispatch 统一记录 `interactive_input_callback/<callback>/frame_entered` trace。该边界只说明 input callback apply 已显式 frame 化，`input_to_get_char_state` gate 已满足，但不能据此执行后台 input callback。
- NOECHO localecho restore 增量：gateway contract 与 gateway command payload 暴露 `interactive_mode_localecho_restore_model=owner_command_frame_localecho_restore`、`interactive_mode_localecho_restore_task_type=interactive_mode_flags`、`interactive_mode_localecho_restore_ready=1`、`interactive_mode_localecho_restore_executor_ready=1` 和 payload 侧 `interactive_mode_localecho_restore_required`；触发路径记录 `interactive_mode_flags/noecho_localecho_restore/frame_detached` trace。该边界说明 NOECHO/localecho restore 已显式 frame 化并可随 gateway command executor 运行，`interactive_mode_flags` gate 已满足，但不能据此开放普通 legacy LPC。
- interactive mode MXP tag filter 增量：gateway contract 与 gateway command payload 暴露 `interactive_mode_mxp_tag_filter_model=owner_command_frame_mxp_tag_filter`、`interactive_mode_mxp_tag_filter_task_type=interactive_mode_flags`、`interactive_mode_mxp_tag_filter_ready=1`、`interactive_mode_mxp_tag_filter_executor_ready=1` 和 payload 侧 `interactive_mode_mxp_tag_filter_required`；触发路径记录 `interactive_mode_flags/mxp_tag_filter/frame_entered` trace。该边界说明 MXP tag filter 已显式 frame 化并可随 gateway command executor 运行，`interactive_mode_flags` gate 已满足，但不能据此开放普通 legacy LPC。
- OLD_ED command frame 增量：gateway contract 与 gateway command payload 暴露 `interactive_mode_ed_command_model=owner_command_frame_ed_command`、`interactive_mode_ed_command_task_type=interactive_mode_flags`、`interactive_mode_ed_command_ready=1`、`interactive_mode_ed_command_executor_ready=1` 和 payload 侧 `interactive_mode_ed_command_required`；触发路径记录 `interactive_mode_flags/ed_command/frame_entered` trace。该边界说明 OLD_ED command 已显式 frame 化并可随 gateway command executor 运行，`interactive_mode_flags` gate 已满足，但不能据此开放普通 legacy LPC。
- prompt write_prompt apply frame 增量：gateway contract 暴露 `command_reply_write_prompt_apply_frame_model=owner_command_frame_write_prompt_apply`、`command_reply_write_prompt_apply_frame_task_type=command_reply`、`command_reply_write_prompt_apply_frame_ready=1`、`command_reply_write_prompt_apply_frame_executor_ready=0`，gateway command payload 暴露 `prompt_write_prompt_apply_frame_*`；触发路径记录 `command_reply/write_prompt_apply/frame_entered` trace。该边界只说明 prompt apply 已显式 frame 化，`prompt_telnet_reschedule_io` 仍是 main-required reply queue，不能据此把网络 reply I/O 迁出主线程。
...
主线程 owner queue 当前承担过渡职责：gateway receive、网络输出、prompt/telnet/reschedule I/O 以及 executor callback cleanup/reply 仍进入 per-owner 队列并在主线程绑定 `VMOwnerScope` 后执行；gateway `process_user_command`、heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 在 `audit`/`enforced` 且 owner thread 可用时优先进入 owner executor，执行前校验 owner epoch，stale/destructed 会 drop 并记录 trace，LPC 引用释放仍通过主线程 cleanup 完成；`socket_release` 因同步 release/acquire 语义仍保留主线程路径。这样能先固定 owner 调度语义和 stale epoch 防线，同时避免把普通 legacy LPC 默认开放。
