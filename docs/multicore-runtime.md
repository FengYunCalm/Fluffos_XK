...
- `owner_local_store_complete_blocker=global_index_bridge_active` 会和 `owner_local_store_complete=0` 一起出现在 object store、owner shard、lookup status 与 shard contract 中；它是当前仍未退休 global index bridge 的机器可读阻塞原因，不代表 owner-local lookup/resolve readiness 失败。
...
- runtime/thread status 的 `vm_context_contract` 暴露 `ordinary_lpc_readiness_gates`、`ordinary_lpc_readiness_gate_count`、`ordinary_lpc_satisfied_gate_count`、`ordinary_lpc_blocked_gate_count`、`ordinary_lpc_next_blocker` 和 `ordinary_lpc_readiness_gate_model=all_gates_required_before_open`。当前 11 项门禁中 8 项已满足、3 项仍阻塞：thread-local VMContext、execution state、owner scope、error state、eval stack owner-local、control stack owner-local、value stack owner-local 和 off-main object store 同步拒绝已满足；其中 eval/control/value stack 都是 thread-local owner execution stack，并通过 VMContext snapshot 在 owner executor task 内 owner-bound、task 后 cleared。仍阻塞普通后台 LPC 的门禁是 apply return、cross-owner object ref 和完整 owner-local object store，下一阻塞点为 `apply_return_owner_local`。因此 `ordinary_lpc_ready=0` 仍是强合同，不允许把受控 allowlist task 扩大成任意 LPC 后台执行。
...
- runtime/thread status 暴露只读 `executor_contract_version=owner_executor_v1`、`executor_model=owner_executor`、`executor_dispatch_model=descriptor_manifest`、`executor_lpc_model=default_closed_allowlist`、`ordinary_lpc_default_policy=default_closed`、`executor_task_contract`、`executor_task_dispatch_contracts` 和 `executor_lpc_task_contracts`：`compute_result`、无目标 `owner_message`、`executor_probe` 和受控注册 `lpc_task` 属于 executor-safe；有 target handle 的 `owner_message` 属于 main-required；普通 `lpc` 属于 rejected/default-closed，但仍由 executor 取走后拒绝，避免 rejected task 卡住 mailbox。`executor_task_dispatch_contracts` 是按 task type 注册的 dispatch manifest，字段包含 `task_type`、`contract_key`、`dispatch_kind`、`executor_runnable`、`executor_safe`、`main_required`、`rejected` 和 route 要求，用来约束 `executor_probe`、`lpc_probe`、`lpc_canary`、`lpc_task`、`lpc`、`owner_state`、`owner_message`、`compute_result` 的调度分支；status、fairness 和 executor trace 会同时暴露 runnable backlog 与 safe backlog，并用 `executor_runnable_task_dispatched`/`executor_safe_task_dispatched` 分别呈现已消费 runnable 数和已安全执行数；executor trace 顶层和事件 mapping 会同步暴露 OwnerExecutor v1 与 descriptor manifest 模型字段，避免把调度 trace 误读成未受控的后台 LPC 执行。`owner_message_mailbox`/`owner_message_target_handle` 合同显式暴露 `requires_owner_mailbox` 和 `requires_owner_main_queue`，与 mailbox task mapping 的 `executor_mode`/`route` 使用同一分类；mailbox task mapping 额外暴露 `task_contract_key`、`task_executor_mode`、`dispatch_kind` 和 `executor_runnable`。`executor_lpc_task_contracts` 是逐方法 descriptor manifest，当前仅注册 `owner_task_readonly`，字段包含 `method`、`executor_mode`、`route`、`result_policy`、target/owner thread/message completion 要求、frozen result 要求和 `direct_cross_owner_write=0`；旧 `lpc_task_allowlist` 数组继续作为兼容只读视图保留。该合同可通过 LPC 层 `vm_owner_runtime_status()`/`vm_owner_thread_status()` 读取，并由 `testsuite/single/tests/efuns/owner_executor_contract.c` 回归覆盖；同一回归也验证 `executor_queue_fairness` 和 `vm_owner_executor_trace()` 的只读结构对 LPC 可见。
