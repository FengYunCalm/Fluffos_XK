...
- ObjectHandle 与 owner shard 状态索引，用于记录 objects、active heartbeats、pending callouts、pending messages、runnable task 数和 executor readiness。
...
owner message/future 当前会同步记录 submitted、completed 和 failed 状态；`owner_call_async()` 会返回目标对象 handle、path 和 owner epoch 快照，供后续 stale handle 校验和真正 message/future 调度使用。
...
cross-owner access 判定会优先使用当前 owner scope 作为 source owner；当 scope 仍是默认 `legacy/main` 且 `command_giver` 已有显式 owner 时，会使用 `command_giver` owner 作为 effective source owner。这覆盖 gateway 命令进入时先以 legacy scope 开始、随后 mudlib 给玩家对象补 owner 的过渡场景，但不放开无 scope 的普通对象调用。
...
在 enforced 模式下，cross-owner `call_other`、`move_object` 和 `destruct` 的同步路径会被阻断，要求迁移到 owner message/future。完整的跨 owner `call_other` message/future 执行链仍在迁移中。
...
- owner payload recursive frozen-data 检查、message/future completed/failed 状态同步、async call 目标 handle 快照。
- owner thread opt-in、上下文绑定、对象表隔离和状态清理。
- owner executor claim/release 生命周期，覆盖同 owner 串行和多 owner 多 worker 分发。
...
- XiaKeXing 运行树短验收：`multicore mode : off` 和 `multicore mode : audit` 的 WebSocket mixed 场景均通过 3 用户登录和命令闭环，且无 timeout、断线、gateway queue full 或 dropped message。
- XiaKeXing `multicore mode : enforced` 当前只验证到启动和登录；mixed 命令已越过命令 owner intent 中读取玩家对象的旧阻断点，但仍会因 server 侧命令 trace、日志和消息 helper 的同步跨 owner `call_other` 被 runtime 拦截而超时，说明业务命令链路还未完成 message/future 或 snapshot 迁移。
...
- object store 仍保留全局 index，owner shard 目前是状态索引和可执行队列观测，不是最终 owner-local object storage。
...
- enforced 模式已经能暴露真实 mudlib 迁移缺口，但不能视为 XiaKeXing 完整可运行模式；目前核心玩家命令仍会在命令 trace、日志或消息发送等 helper 路径命中同步跨 owner 调用拦截。
