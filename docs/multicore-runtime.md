...
- runtime/thread status 的 `gateway_owner_task_contract` 暴露 input/gateway 的当前执行边界：`input_model=owner_executor_with_main_fallback`、`executor_migration_state=owner_executor_active`、`ordinary_lpc_ready_required=0`，并把 `process_user_command` 固定为 `executor_safe_callback` / `owner_executor` 合同。gateway receive、logon、disconnected、prompt/telnet/reschedule I/O 和网络 reply 仍保留主线程 owner queue 边界；真实玩家命令执行走 `gateway_command_execute` / `process_user_command` executor callback，使用 owner-private command snapshot、execution-frame restore、owner epoch target-handle guard 和 stale drop trace。7 项 command executor readiness gate 与 5 项 side-effect gate 均已满足，`gateway_command_executor_activation` 已进入 guarded executor-safe 合同。普通 LPC 显式开放路径仍不等于任意 legacy LPC 默认开放；gateway command executor 只通过 driver callback allowlist 执行玩家命令，prompt/telnet/reschedule reply queue 仍是 main-required 网络 I/O 边界。
- production gate 状态合同：`gateway_owner_task_contract` 与 `owner_executor_boundary_contract` 暴露 `mudlib_audit_required=1`、`mudlib_cross_owner_hotspots_ready=1`、`mudlib_cross_owner_hotspots_blocker=""`、`mudlib_cross_owner_hotspots_evidence=xkx_5513c8a12_multicore_mudlib_audit_2026_06_25_zero_delayed_object_payloads`、`production_gate_ready=1`、`production_gate_blocker=""`、`production_gate_required_users=1,3,10`、`production_gate_required_durations=smoke,30m`、`production_gate_pressure_evidence_ready=1`、`production_gate_pressure_evidence=xkx_audit_10_users_30m_2026_06_25_zero_timeouts_zero_gateway_errors`、`production_gate_socket_release_policy=owner_safe_synchronous_release_acquire_handshake` 和 `production_gate_socket_release_handshake_ready=1`。这些字段说明当前压力口径、真实 XiaKeXing mudlib final audit 和 socket release/acquire owner epoch guard 均已收口。
...
- `lpc_task`：普通方法仍默认关闭；descriptor 注册的 18 个 `owner_task_*` domain task 显式开放到 `executor_safe_allowlist`，包括 `owner_task_readonly`、`owner_task_player`、`owner_task_room`、`owner_task_session`、`owner_task_item`、`owner_task_economy`、`owner_task_combat`、`owner_task_mail`、`owner_task_reward`、`owner_task_world`、`owner_task_persistence`、`owner_task_team`、`owner_task_guild`、`owner_task_sect`、`owner_task_quest`、`owner_task_rank`、`owner_task_crafting` 和 `owner_task_life_skill`。提交后会返回可轮询 `future_id`、`registered_task`、`task_contract`、`executor_mode`、`route`、`result_policy` 和 `contract_reason`；线程侧成功时写入 frozen result，未注册方法的拒绝、失败、purge 或被主线程兼容 drain/schedule 消费时都会完成为 failed，避免 pending future 堆积。
...
- 普通 LPC 任务默认拒绝，descriptor 注册的 18 个 `owner_task_*` domain task 可在 owner thread 执行并返回 frozen result。
- restricted canary、注册 owner LPC domain task 执行、未注册 owner LPC task 拒绝，以及普通 legacy LPC 仍保持 default-closed。
...
- `socket_release` callback 仍保持同步路径，因为 efun 返回值依赖 callback 是否立即完成 release/acquire 交接；该同步路径已捕获 release 目标 owner epoch，并在 `socket_acquire()` 时校验同一 owner epoch，成功、拒绝、stale 和 owner mismatch 都写入 task trace。
