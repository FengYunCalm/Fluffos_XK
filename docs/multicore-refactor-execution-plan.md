...
- `vm_owner_runtime_status()` 与 `vm_owner_thread_status()` 额外暴露 `owner_executor_boundary_contract`，把 OwnerExecutor 边界固定为机器可读合同：`implementation_state=embedded_class_active`、`class_extracted=1`、`module_extracted=0`，说明当前 `OwnerExecutor` 已是 `owner.cc` 内部类并由 `owner_thread_loop()` 使用，但还没有拆成独立模块；claim/release、budget、thread context、dispatch manifest、same-owner serial、target-handle main-required、`compute_result` executor-safe、`gateway_command` rejected、普通 LPC default-closed/explicit-open 都已作为边界输入；下一步目标是 `extract_owner_executor_module_without_expanding_lpc_surface`，不是扩大 gateway command 或任意 LPC 后台执行面。
