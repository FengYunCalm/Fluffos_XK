...
- 玩家输入、gateway、socket callback、ED callback 仍以主线程 owner queue 作为兼容入口；heartbeat、callout、async/db/file completion 和 DNS callback 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；callout、async 和 DNS 的成功执行、stale/drop 和 destructed 清理都回主线程 cleanup 释放 LPC 引用。
...
- OwnerExecutor callback task boundary 已完成：`heartbeat`、`call_out`、`async_callback`、`dns_callback`、`socket_callback`、`gateway_command_execute` 进入 driver callback allowlist；其中 heartbeat、callout、async/db/file completion 和 DNS callback 已绑定真实入口并具备 stale owner/epoch drop，socket 和 gateway command execute 入口尚未迁成真实生产执行路径。
- input/socket/gateway command 的真正后台并行执行；heartbeat、callout、async 和 DNS 已具备 owner executor dispatch，但仍需纳入后续真实 mudlib 长压。
