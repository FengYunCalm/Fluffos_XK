...
- owner-local lookup/resolve object store gate 已完成；create/move/destruct canonical 写入已通过 lifecycle 合同标记为 ready，但 `owner_local_deferred_destruct_ready=0`、`global_index_physical_retirement_ready=0`，所以完整对象生命周期、deferred destruct 和全局索引物理迁出的生产级并发化仍未完成。
