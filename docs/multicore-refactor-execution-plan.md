...
- 玩家输入、gateway、async/db/file completion、DNS callback、socket callback、ED callback 仍以主线程 owner queue 作为兼容入口；heartbeat 和 callout 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；callout 的成功执行、stale/drop 和 destructed 清理都回主线程 cleanup 释放 LPC 引用。
...
- OwnerExecutor callback task boundary 已完成：`heartbeat`、`call_out`、`async_callback`、`dns_callback`、`socket_callback`、`gateway_command_execute` 进入 driver callback allowlist；其中 heartbeat 和 callout 已绑定真实到期入口并具备 stale owner/epoch drop，async/DNS/socket 和 gateway command execute 入口尚未迁成真实生产执行路径。
- input/async/DNS/socket/gateway command 的真正后台并行执行；heartbeat 和 callout 已具备 owner executor dispatch，但仍需纳入后续真实 mudlib 长压。
...
11. heartbeat 和 callout 已迁到 owner executor；下一步迁移 async/db/file/DNS/socket callback。
...
这些基础已经支撑 heartbeat 和 callout 真实入口迁到 owner executor；当前最小后续任务是迁移 async/DNS/socket callback，再推进真实 gateway command 执行。
