...
- OwnerExecutor callback task boundary 已完成：`heartbeat`、`call_out`、`async_callback`、`dns_callback`、`socket_callback`、`gateway_command_execute` 进入 driver callback allowlist，但这些入口尚未迁成真实生产执行路径。
