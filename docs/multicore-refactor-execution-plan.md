...
- 玩家输入、gateway、callout、async/db/file completion、DNS callback、socket callback、ED callback 仍以主线程 owner queue 作为兼容入口；heartbeat 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue。
...
- OwnerExecutor callback task boundary 已完成：`heartbeat`、`call_out`、`async_callback`、`dns_callback`、`socket_callback`、`gateway_command_execute` 进入 driver callback allowlist；其中 heartbeat 已绑定真实到期入口并具备 stale owner/epoch drop，其余 callout、async/DNS/socket 和 gateway command execute 入口尚未迁成真实生产执行路径。
- callout/input/async/DNS/socket/gateway command 的真正后台并行执行；heartbeat 已具备 owner executor dispatch，但仍需纳入后续真实 mudlib 长压。
...
目标：让世界 tick 能按 owner 分片并行。当前 heartbeat 已完成第一条真实 owner executor 路径：到期 tick 在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；callout 仍待迁移。
...
4. owner executor 执行到期 callout，并保持同 owner 顺序。
...
- 不同 owner heartbeat 可并行的长期吞吐仍纳入真实 mudlib 长压。
...
11. heartbeat 已迁到 owner executor；下一步迁移 callout。
...
这些基础已经支撑 heartbeat 真实入口迁到 owner executor；当前最小后续任务是迁移 callout，再依次推进 async/DNS/socket callback 和真实 gateway command 执行。
