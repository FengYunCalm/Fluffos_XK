...
- 玩家输入、gateway 和 ED callback 仍以主线程 owner queue 作为兼容入口；heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；callout、async、DNS 和 socket callback 的成功执行、stale/drop/destructed 清理都回主线程 cleanup 释放 LPC 引用；`socket_release` 因同步 release/acquire 语义仍保留主线程路径。
...
- OwnerExecutor callback task boundary 已完成：`heartbeat`、`call_out`、`async_callback`、`dns_callback`、`socket_callback`、`gateway_command_execute` 进入 driver callback allowlist；其中 heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 已绑定真实入口并具备 stale owner/epoch drop，gateway command execute 入口尚未迁成真实生产执行路径。
- input/gateway command 的真正后台并行执行；heartbeat、callout、async、DNS 和 socket callback 已具备 owner executor dispatch，但仍需纳入后续真实 mudlib 长压。
...
当前状态：async/db/file completion 与 DNS callback 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；成功执行、stale/drop 和 destructed 清理都走主线程 cleanup。剩余生产入口是 socket read/write/close callback，`socket_release` 仍保持 main-required 例外。
...
10. input/gateway 前置 gate 和 guarded activation 已完成，真实玩家命令执行仍待独立生产 gate。
11. heartbeat、callout、async/db/file completion 和 DNS callback 已迁到 owner executor；下一步迁移 socket callback。
12. 再迁移真实 gateway command executor 执行路径。
