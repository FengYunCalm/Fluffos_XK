...
- 即使 object ref 边界和 owner-local lookup/resolve store gate 已完成，普通 LPC 仍必须保持默认关闭，直到 activation policy 明确打开并通过生产级验收。
