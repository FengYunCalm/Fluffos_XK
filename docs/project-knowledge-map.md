...
- `owner_executor_boundary_contract` 已在 runtime/thread status 暴露 OwnerExecutor 边界合同：`implementation_state=header_module_active`、`class_extracted=1`、`module_extracted=1`、`module_file=vm/internal/owner_executor.h`、`compilation_unit_extracted=0`，说明当前 `OwnerExecutor` 已从 `owner.cc` 抽成内部模块文件并由 `owner_thread_loop()` 使用，但仍依赖 `owner.cc` 匿名命名空间运行时状态，尚未拆成独立编译单元；claim/release、budget、thread context、dispatch manifest、same-owner serial、target-handle main-required、`compute_result` executor-safe、`gateway_command` rejected、普通 LPC default-closed/explicit-open 均被固定；下一步是抽出独立 OwnerExecutor 编译单元，不是扩大 gateway command 或任意 LPC 后台执行面。
