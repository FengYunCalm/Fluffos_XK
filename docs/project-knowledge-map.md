...
- socket callbacks 仍投递到主线程 owner queue，尚未迁到 owner executor。
...
- socket/gateway command 的真实 owner executor 执行路径；heartbeat、callout、async/db/file completion 和 DNS callback 真实入口已迁到 owner executor，但仍需真实 mudlib 长压验证吞吐。
- array、mapping、object ref 的完整跨线程内存模型。
- driver callback allowlist 到剩余入口实际迁移的生产路径。
- input/socket/gateway command 在 owner executor 中真正并行执行。
