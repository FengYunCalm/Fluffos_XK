...
- `vm_context_contract` 已在 runtime/thread status 暴露普通后台 LPC readiness gate：11 项门禁中 thread-local VMContext、execution state、owner scope、error state、eval stack owner-local、control stack owner-local、value stack owner-local 和 off-main object store 同步拒绝已满足；apply return、cross-owner object ref 和完整 owner-local object store 仍阻塞，因此 `ordinary_lpc_ready=0` 仍是强合同。
...
- apply return、array、mapping、object ref 的完整跨线程内存模型。
