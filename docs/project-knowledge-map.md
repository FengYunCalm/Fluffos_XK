...
- callout/async/DNS/socket/gateway command 的真实 owner executor 执行路径；heartbeat 真实入口已迁到 owner executor，但仍需真实 mudlib 长压验证吞吐。
- array、mapping、object ref 的完整跨线程内存模型。
- driver callback allowlist 到剩余入口实际迁移的生产路径。
- callout/input/async/DNS/socket/gateway command 在 owner executor 中真正并行执行。
