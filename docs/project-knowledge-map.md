...
- `owner_executor_boundary_contract` 已在 runtime/thread status 暴露 OwnerExecutor 边界合同：`implementation_state=embedded_class_active`、`class_extracted=1`、`module_extracted=0`，说明当前 `OwnerExecutor` 已是 `owner.cc` 内部类并由 `owner_thread_loop()` 使用，但还没有拆成独立模块；claim/release、budget、thread context、dispatch manifest、same-owner serial、target-handle main-required、`compute_result` executor-safe、`gateway_command` rejected、普通 LPC default-closed/explicit-open 均被固定；下一步是抽出独立 OwnerExecutor 模块，不是扩大 gateway command 或任意 LPC 后台执行面。
