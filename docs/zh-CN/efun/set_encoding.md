...
### 名称 / NAME
    ...
    set_encoding() - set output/input encoding for current player
...
### 描述 / DESCRIPTION
    ...
    If the given encoding name is not available, an error will be thrown. Available names depend on
    your ICU version. GBK, GB2312, Big5, and UTF-8 are common boundary encodings for Chinese mudlibs
    and clients.

    如果没有指定编码，会重置玩家的编码为 UTF-8。

    If no encoding is specified, the player session is reset to UTF-8.

    返回值为 ICU 规范的编码名称，与 query_encoding() 返回值相同。

    The return value is the canonical ICU encoding name and matches query_encoding().
    ...
    to the player. For non-UTF-8 legacy LPC source files, use a source boundary declaration such as
    #pragma source_encoding("GBK"). For external data buffers, use string_encode(), string_decode(),
    or buffer_transcode(); do not mix external byte encodings into internal VM string semantics.
