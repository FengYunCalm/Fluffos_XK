...
| A-S1 | 每提交定向测试 + GTest + 444/444（2026-08-19 实测，见 evidence；424 为计划基线）+ ASan/UBSan；阶段末 TSan（2026-08-19 注记：TSan 构建 OOM 未跑，WSL2 内存限制，deferred——见 evidence 文档） |
| A-S2 | 全矩阵状态闭合 + GTest + 444/444（同上）+ ASan/UBSan/TSan（TSan deferred，同上） |
...
| C-S1 | RAII 覆盖 parse error/C++异常 + arena GTest + 3 轮 randomized 424/424 + ASan/UBSan + 全旧符号 grep 零命中（2026-08-19 注记：实际执行 lpc_tests 444/444 单轮 + 全量 -ftest；arena GTest 未建（compile_arena 由 bench 工具与 mud_status 消费）；ASan/UBSan 已复验——**ASan 全量 lpc_tests 444/444 PASSED（SimulEfunReload 指针序断言修复后，6316f593），无 UAF/double-free/orphan**——见 evidence 文档） |
