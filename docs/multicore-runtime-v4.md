...
performance thresholds. Recording is disabled by default and must be enabled
explicitly for the current VM thread by a diagnostic benchmark or test;
multicore audit/enforced mode does not turn continuous profiling on.

Runtime v4 通过 `lpc_vm_profile_v1` 和 `lpc_vm_bench_v1` 暴露热路径 profiling 计数。
这些是诊断信号，不是依赖具体机器的性能硬阈值。记录默认关闭，只能由诊断
benchmark 或测试对当前 VM 线程显式开启；multicore audit/enforced 模式不会自动
启用持续 profiling。
