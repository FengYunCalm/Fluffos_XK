...
- 主线程 owner queue，仍接管 gateway/player input 和 main-required cleanup/reply；到期 heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 在 `audit`/`enforced` 且 owner thread 可用时优先走 owner executor，`off` 或 executor unavailable 时回退 owner main queue；`socket_release` 因同步 release/acquire 语义仍保留主线程路径。
...
- interactive/gateway input 已先进入主线程 owner queue，且 `current_object`、`current_prog`、`previous_ob`、`caller_type`、`call_origin`、inherit offset、stack temporary depth、`current_interactive`、`command_giver`、`current_error_context`、eval error flags、error handler depths 和 object lifecycle guard 高频入口已收敛到 VMContext API；真实 gateway command executor 执行与完整跨线程 value/object ref 模型仍未完成。
