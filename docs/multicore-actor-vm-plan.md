...
- interactive/gateway input 已先进入主线程 owner queue，且 `current_object`、`current_prog`、`previous_ob`、`caller_type`、`call_origin`、inherit offset、stack temporary depth、`current_interactive`、`command_giver`、`current_error_context`、eval error flags 高频入口已收敛到 VMContext API；eval stack 和 object store 仍未完全 owner-local 化。
