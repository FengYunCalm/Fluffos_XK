...
- object store 仍是全局模型。
- heartbeat/callout 仍未 owner-local。
- interactive/gateway input 还未完全 owner queue 化。
- `call_other` 仍可形成跨 owner 同步调用。
- array/mapping/object ref 跨线程内存模型还未封口。
- async/db/file/network 回调还未统一投递到 owner executor。
...
- 任务类型统一覆盖 command、heartbeat、callout、apply、message、async completion、gateway input、maintenance。
...
- 多个 XiaKeXing 玩家命令可按 session owner 并行。
- 单个玩家命令仍串行。
...
- backend 只推进时间和唤醒 owner。
- 到期 heartbeat/callout 由 owner executor 执行。
...
4. owner shard。
5. OwnerExecutor。
6. interactive/gateway owner 化。
7. heartbeat/callout owner-local。
8. cross-owner call audit。
9. message/snapshot/future。
10. frozen payload。
11. async/db/file/network owner 化。
12. VM worker 并入 future。
13. XiaKeXing audit 迁移。
14. XiaKeXing enforced 运行。
15. 清理旧全局状态。
