...
- `current_object`/`current_prog`/`previous_ob`/`caller_type` execution frame setter，`call_origin` setter，`command_giver` 的 VMContext setter/保存栈同步，`current_interactive` 的 VMContext setter/scope，以及 `current_error_context` 的 VMContext 同步，已用于 apply/direct/function pointer、interactive/gateway 高频入口和 `this_player()` 读取路径。
- owner thread opt-in。
...
- interactive/gateway input 已先进入主线程 owner queue，且 `current_object`、`current_prog`、`previous_ob`、`caller_type`、`call_origin`、`current_interactive`、`command_giver`、`current_error_context` 高频入口已收敛到 VMContext API；eval stack 和 object store 仍未完全 owner-local 化。
