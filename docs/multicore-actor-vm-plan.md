...
- ObjectHandle、owner shard 状态索引、pending message/callout/heartbeat 可执行状态观测。
- 主线程 owner queue，仍接管 gateway receive、network reply、prompt/telnet/reschedule I/O 和 main-required cleanup/reply；gateway `process_user_command`、到期 heartbeat、callout、async/db/file completion、DNS callback 和 socket read/write/close callback 在 `audit`/`enforced` 且 owner thread 可用时优先走 owner executor，`off` 或 executor unavailable 时回退 owner main queue；`socket_release` 因同步 release/acquire 语义仍保留主线程路径。
- `current_object`/`current_prog`/`previous_ob`/`caller_type` execution frame setter，`call_origin`、inherit offset 和 stack temporary depth setter，`command_giver` 的 VMContext setter/保存栈同步，`current_interactive` 的 VMContext setter/scope，`current_error_context`、eval error flags 和 error handler depths 的 VMContext 同步，以及 object lifecycle guard 同步，已用于 apply/direct/function pointer、interactive/gateway 高频入口和 `this_player()` 读取路径；detached `VMContext` setter/apply/sync 不再污染当前线程 thread-local execution state。
- `vm_context_reset_execution()` 会同步清理当前线程 thread-local execution state 和 VMContext snapshot。
- owner thread opt-in。
- off-main LPC task 默认关闭；仅保留受控 probe/canary 验证路径。
- owner executor claim/release 生命周期计数，可验证同 owner 串行 claim 释放。
- owner payload 递归 frozen-data 检查、owner message/future 状态同步、`owner_call_async()` 目标 ObjectHandle 快照返回。
- enforced 模式已阻断 cross-owner `call_other`、`move_object`、`destruct` 的同步路径，要求迁移到 message/future。
- 加载型 singleton 对象默认归入 `legacy/main`，避免命令对象、daemon 等共享对象被首次触发加载的玩家 owner 污染；clone 仍继承当前 owner 或 prototype owner。
- XiaKeXing 运行树已在 `multicore mode : off`、`audit` 和 `enforced` 下通过短 WebSocket mixed 验收；最新 enforced 3 用户 mixed 结果为 3/3 登录、109/109 命令成功、0 timeout、0 断线。
- Gateway session 集成。
...
- owner-local object lifecycle 已完成：create/move/destruct/deferred destruct canonical write 归 owner shard，global index 只保留诊断、兼容统计和 bridge 一致性检查。
- heartbeat/callout/async/db/file completion/DNS callback 和 socket read/write/close callback 已迁入 owner executor：`audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；socket callback args 采用 deep-copy/frozen payload policy，success/drop cleanup 仍回主线程释放 LPC 引用；`socket_release` 保持 main-required；长期吞吐仍需真实 mudlib 压测证明。
- gateway receive 仍先进入主线程 owner queue，`process_user_command` 已在 `audit`/`enforced` 且 owner thread 可用时通过 `gateway_command_execute` owner executor callback 执行，`current_object`、`current_prog`、`previous_ob`、`caller_type`、`call_origin`、inherit offset、stack temporary depth、`current_interactive`、`command_giver`、`current_error_context`、eval error flags、error handler depths 和 object lifecycle guard 高频入口已收敛到 VMContext API；剩余边界是完整跨线程 value/object ref 模型、真实 mudlib audit 和长压验收。
- enforced 模式已阻断跨 owner 同步 `call_other`，并会优先使用当前 owner scope 或默认 `legacy/main` scope 下的显式 `command_giver` owner 作为访问判定 source owner；完整替换为可执行 message/future 调用链仍未完成。
- XiaKeXing `multicore mode : enforced` 已通过短 WebSocket mixed 命令闭环，但这只是 3 用户短验收，不等于完整生产压测，也不代表旧 mudlib 的所有跨 owner 同步路径都已完成 message/future 迁移。
- array/mapping/object ref 跨线程内存模型还未封口。
- async/db/file completion、DNS callback 和 socket read/write/close callback 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，并通过主线程 cleanup 释放 LPC 引用；`socket_release` callback 因同步返回语义仍保留原路径。
...
- 当前实测：XiaKeXing enforced 3 用户 WebSocket mixed 短验收已通过，最新结果为 109/109 命令成功、0 timeout、0 断线；该结果依赖命令/daemon singleton 不再继承首个玩家 owner。
...
- async/db/file completion 记录 callback owner id/epoch，并在 owner thread 可用时投递 owner executor。
- DNS callback 记录 callback owner id/epoch，并在 owner thread 可用时投递 owner executor。
- socket read/write/close callback 记录 callback owner id/epoch，deep-copy/freeze 参数，并在 owner thread 可用时投递 owner executor。
- async/DNS/socket owner 已销毁或 epoch 不匹配时 drop、记录 trace，并派发主线程 cleanup。
- `socket_release` 保持 main-required，同步返回语义不在本阶段绕开。
...
- async/DNS/socket I/O 回调不会跨 owner 修改对象。
- async/DNS/socket executor success/drop cleanup 均在主线程释放 LPC 引用。
- socket read/write/close callback 状态合同报告 `socket_owner_executor_ready=1`，`socket_release_main_required=1` 仍保持主线程例外。
...
- 当前实测：`off`、`audit` 和 `enforced` mixed 场景均通过短验收；最新 `enforced` 3 用户 mixed 为 3/3 登录、109/109 命令成功、0 timeout、0 断线。
- enforced 模式下更长时间、多场景和更高并发的 XiaKeXing 压测仍是未完成目标。
...
11. frozen payload。
12. async/db/file/DNS/socket callback 已 owner executor 化；下一步迁真实 gateway command 执行。
