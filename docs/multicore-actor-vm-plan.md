...
- XiaKeXing 运行树已在 `multicore mode : off` 和 `multicore mode : audit` 下通过短 WebSocket mixed 验收：3/3 登录、命令全成功、无超时、无断线、无 gateway queue 丢弃。
- Gateway session 集成。
...
- XiaKeXing `multicore mode : enforced` 当前能启动并登录，但 mixed 命令验收仍失败。此前命令 owner intent 记录链路中的玩家对象读取误拦已越过，当前阻断推进到 `command_dispatch_d.c::record_owner_command_trace()` 经 worker adapter 记录 owner task，以及 `log_file`/`msg_send` helper 的同步跨 owner `call_other`。该结果说明 server 侧命令 trace、日志和消息发送仍需迁移到 snapshot 或 message/future，不能把 enforced 运行标记为完成。
...
- 当前实测：XiaKeXing enforced mixed 命令已越过命令 owner intent 里的玩家对象读取误拦，但会在命令 trace、日志和消息 helper 的同步跨 owner `call_other` 上继续被拦截，核心交互路径尚未完成替代。
...
- 当前实测：`off` 和 `audit` mixed 场景通过短验收；`enforced` 可启动和登录，但玩家命令全部超时，当前根因推进到 server 命令 trace、日志和消息发送路径仍存在同步跨 owner `call_other`。
- enforced 模式下核心登录、移动、查看、基础战斗可跑仍是未完成目标。
