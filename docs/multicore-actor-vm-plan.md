...
- enforced 模式已阻断跨 owner 同步 `call_other`，但完整替换为可执行 message/future 调用链仍未完成。
- XiaKeXing `multicore mode : enforced` 当前能启动并登录，但 mixed 命令验收失败：`owner_gameplay_d.c::query_actor_id()` 在命令 owner intent 记录链路中同步读取玩家对象，被 enforced cross-owner `call_other` 拦截。该结果说明 server 侧 owner intent/玩家状态读取仍需迁移到 snapshot 或 message/future，不能把 enforced 运行标记为完成。
...
- 当前实测：XiaKeXing enforced mixed 命令会在 `owner_gameplay_d.c::query_actor_id()` 同步跨 owner 读取玩家对象时被拦截，核心交互路径尚未完成替代。
...
- 当前实测：`off` 和 `audit` mixed 场景通过短验收；`enforced` 可启动和登录，但玩家命令全部超时，根因是 server 命令 owner intent 记录仍存在同步跨 owner `call_other`。
