...
- 主线程 owner queue，已接管 gateway/player input、到期 callout、heartbeat、async/db/file completion 的 owner-bound 分发。
- owner thread opt-in。
...
- interactive/gateway input 已先进入主线程 owner queue，但 `this_player()`、`current_interactive` 等执行状态仍未 owner-local 化。
- `call_other` 仍可形成跨 owner 同步调用。
- array/mapping/object ref 跨线程内存模型还未封口。
- async/db/file completion 已先进入主线程 owner queue；socket/DNS 等网络回调还未统一投递到 owner executor。
...
- 任务类型统一覆盖 command、heartbeat、callout、apply、message、async/db/file completion、gateway input、network callback、maintenance。
