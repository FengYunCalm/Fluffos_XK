...
- `command_giver` 的 VMContext setter/保存栈同步，以及 `current_interactive` 的 VMContext setter/scope，已用于 interactive/gateway 高频入口和 `this_player()` 读取路径。
- owner thread opt-in。
...
- interactive/gateway input 已先进入主线程 owner queue，且 `current_interactive`、`command_giver` 高频入口已收敛到 VMContext API；更深 apply 状态仍未完全 owner-local 化。
