...
- 主线程 owner queue，仍接管 gateway/player input、socket read/write/close callback 和 main-required cleanup/reply；到期 heartbeat、callout、async/db/file completion 和 DNS callback 在 `audit`/`enforced` 且 owner thread 可用时优先走 owner executor，`off` 或 executor unavailable 时回退 owner main queue。
...
- heartbeat/callout/async/db/file completion/DNS callback 已迁入 owner executor：`audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；长期吞吐仍需真实 mudlib 压测证明。
...
- async/db/file completion 和 DNS callback 已在 `audit`/`enforced` 且 owner thread 可用时投递 owner executor，并通过主线程 cleanup 释放 LPC 引用；socket read/write/close callback 仍在主线程 owner queue，`socket_release` callback 因同步返回语义仍保留原路径。
...
- async/DNS owner 已销毁或 epoch 不匹配时 drop、记录 trace，并派发主线程 cleanup。
- socket read/write/close callback 仍需迁到 owner executor。
...
- async/DNS I/O 回调不会跨 owner 修改对象。
- async/DNS executor success/drop cleanup 均在主线程释放 LPC 引用。
- 剩余 socket callback 迁移前，socket read/write/close 仍被文档和状态合同标为未完成。
...
11. frozen payload。
12. async/db/file/DNS 已 owner executor 化；下一步迁 socket read/write/close callback。
