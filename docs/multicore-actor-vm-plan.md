...
- `current_interactive` 的 VMContext setter/scope，已用于 interactive/gateway 高频入口。
- owner thread opt-in。
...
- interactive/gateway input 已先进入主线程 owner queue，且 `current_interactive` 高频入口已收敛到 VMContext API；`this_player()`、`command_giver` 和更深 apply 状态仍未完全 owner-local 化。
