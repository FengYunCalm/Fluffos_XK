...
- `current_object`/`current_prog`/`previous_ob`/`caller_type` execution frame setter，`command_giver` 的 VMContext setter/保存栈同步，以及 `current_interactive` 的 VMContext setter/scope，已用于 apply/direct/function pointer、interactive/gateway 高频入口和 `this_player()` 读取路径。
- owner thread opt-in。
...
- interactive/gateway input 已先进入主线程 owner queue，且 `current_object`、`current_prog`、`previous_ob`、`caller_type`、`current_interactive`、`command_giver` 高频入口已收敛到 VMContext API；eval stack 和 object store 仍未完全 owner-local 化。
