...
- object store 仍保留全局 index，但已有 owner shard bookkeeping；尚未变成真正 owner-local object storage。
- heartbeat/callout 已迁入 owner executor：`audit`/`enforced` 且 owner thread 可用时投递 owner executor，`off` 或 executor unavailable 时回退 owner main queue；长期吞吐仍需真实 mudlib 压测证明。
- interactive/gateway input 已先进入主线程 owner queue，且 `current_object`、`current_prog`、`previous_ob`、`caller_type`、`call_origin`、inherit offset、stack temporary depth、`current_interactive`、`command_giver`、`current_error_context`、eval error flags、error handler depths 和 object lifecycle guard 高频入口已收敛到 VMContext API；eval stack 和 object store 仍未完全 owner-local 化。
