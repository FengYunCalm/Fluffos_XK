...
| 工作包 | 状态 | 交付物/证据 |
| --- | --- | --- |
| T01 sys_reload_tls 边界 | done | 边界修复（T01）＋ R2-F12 管理合同：主线程强制、master 授权 hook（`valid_sys_reload_tls`）、固定检查顺序（线程→授权→索引→TLS 类型）、失败原子性（旧 SSL_CTX 保持）。证据：`DriverTest.TestSysReloadTlsPermissionAndThreadMatrix` 覆盖 main+authorized/unauthorized、websocket/non-TLS、极值索引、worker+authorized 拒绝、reload 失败原子性；`testsuite/single/tests/efuns/sys_reload_tls.c`；`SECURITY.md` 与 `docs/runbooks/gateway-security.md` 已更新。本地 `lpc_tests` 420/420 通过 |
| T02 Release DAG 重排 | needs-remediation | R2-F08：target SHA 精确校验与直接依赖、JSON required-check 全集（18 项）、check-runs 分页/重跑按最新记录裁决、单次构建 OCI archive + skopeo digest + Trivy 阻断 + 同 digest 推广（远端 inspect 复核）、verify-release-inputs 只读 job、最小权限（仅 publish-draft/promote-image 在 protected environment 有写权限）、全局 release concurrency、dry_run 模式与故障注入（`tools/release/release-fault-injection.sh` 全过）。`actionlint 1.7.12` 全部 workflow 通过。剩余：真实 registry 受保护演练需发布时执行 |
| T03 证据登记 | done | R2-F07：envelope 专用 artifact（`capacity-evidence-envelope`）+ raw 分流；validator/gate 分层；UTC 时间有序、future-skew、cleanup=clean、evidence_kind=current、source/tested SHA 由 workflow context 传入、run_id 唯一、raw SHA-256 绑定；`tools/docs/test-evidence-gate.py` 全部负例拒绝；当前 HEAD 报告提交于 `docs/reports/capacity-evidence-2026-08-11/` |
| T04 LPC 隔离 runner | done | R2-F11：driver 支持端口 0（comm.cc getsockname 回写、sys_network_ports/日志暴露实际端口）；runner 直配 0（无 probe/close/rebind 窗口）、配置渲染校验、fallback lock 带 PID/时间/陈旧识别/trap 清理；端口契约在正常启动模式验证（四个唯一 loopback 端口）；CI 已消费 runner（--mode audit）；`test-port-isolation.sh`：5 串行 + 20 并发零冲突零残留（本地执行通过） |
| T05 Sanitizer 矩阵 | needs-remediation | 矩阵结构与 TSan 子集断言已就绪（check_name 唯一化，R2-F01）；本地 GCC ASan Debug 420/420、UBSan Debug 420/420、TSan focused subset 360/360 通过。TSan 在当前 WSL 需用 `setarch x86_64 -R` 固定地址布局；Clang/macOS/Windows 仍待当前新 HEAD 远端 CI |
| T06 CMake 生成污染 | needs-remediation | 改动已落地；本地同一 build 目录连续构建，第二次无 C++ 编译/链接且源码工作树未新增生成物。跨平台最终确认仍随当前新 HEAD 远端矩阵执行 |
| T07 可复现构建 | needs-remediation | CMakePresets 已落地；无 artifact reproducibility 证据 |
| T08 供应链 manifest | done | R2-F09：全部 actions 以 commit SHA pin（14 项结构化记录：name/version/sha/upstream/review_date）、alpine:3.18 与 trivy:0.50.1 以 digest pin（registry 解析）、Dockerfile FROM digest + jemalloc 下载校验解压（禁 pipe-to-tar）、checker 强制 SHA/digest/无 pipe-to-tar 且 `--self-test` 负例全过、CI 跑 `sbom-generate --validate` |
| T09 安全策略 | needs-remediation | SECURITY.md、私钥打包检查、external bind 拒绝已落地；CodeQL 与镜像漏洞阻断策略（Trivy 已落地 R2-F08）的远端 run 待验证 |
| T10 OwnerExecutor 异常 | needs-remediation | 异常终态化/RAII 已落地；本地 GCC TSan focused subset 360/360 通过，远端 Clang TSan 仍待当前新 HEAD CI |
| T11 ObjectHandle 引用 | done | R2-F06：主线程 admission 全入口强制（handle 与 raw 路径统一拒绝 `main_thread_admission_required`，release 构建生效）、`VMObjectRefGuard` 拆为 `release_ref`/`detach`、worker ref-mutation 探针只计实际 off-main 变更并先做非零自校准、worker resolver 只读取 owner-local 生命周期/指针索引且不接触 compatibility/global bridge。证据：`TestVmOwnerWorkerNestedObjectTargetSubmissionRejected`、`TestVmOwnerWorkerNeverMutatesObjectRefcounts`、compatibility-only 对象 generic 可见但 owner-local resolver 拒绝；420/420 通过，本地 ASan/UBSan/TSan 复验通过 |
| T12 Future 回收 | done | R2-F05：生命周期槽位预留（admission 成功→终态可查询不变量）、quota 拒绝保留 failed 墓碑（poll/state/take 可见直至 take/TTL）、quota 返回仍在解锁后解析真实 target status、insert 拆分 admit_pending/restore_terminal_checked、frozen 计量覆盖 mapping key/value 与 MSTR_SIZE、深度/节点/饱和上限、all-terminal 与 reapable 双时间索引、全入口共享记账。证据：`OwnerFutureStoreTest` 15 项含并发压力、`PayloadByteCapsRejectDeterministically` 的 invalid-target 回归、`TestFutureFrozenMappingKeyAndValueBytesAreCounted`、容量 envelope（`docs/reports/capacity-evidence-2026-08-11/`） |
| T13 Gateway 扫描观测 | needs-remediation | R2-F10：指标改为事件精确计数（scan_runs_total/ready_selected_total/continuation_scheduled_total/coalesced_requests/callbacks_executed/sessions_flushed 等 10 项），`TestGatewayMasterOutputContinuationScheduleSemantics` 断言调度/合并/回调增量；按审计要求，1K/4K/16K session 报告产生前保持 needs-remediation |
| T14 Scheduler 容量 | done | `tools/owner-scheduler-capacity.sh` 重构（R2-F07）：UUID run_id、SHA-256 build hash、cleanup 由 bench 指标推导（`tools/docs/derive-cleanup.py`）、raw digest 绑定、gate 全过；当前 HEAD 报告提交于 `docs/reports/capacity-evidence-2026-08-11/`（3 envelope + 3 raw，digest 校验通过） |
| T15 边界与 fuzz | needs-remediation | R2-F02/F04：fuzz 目标拆分（默认构建不含无 main 目标、非 Clang 开开关 configure 失败）、每个场景独立 fixture（fd 重解析、peer 显式释放、空输入安全、init 失败 abort）、smoke 硬断言（256 inputs、1024 accepted frames、256 length rejects、512 JSON rejects、零 master/pending 残留）、corpus 提交、CI fuzz 使用绝对 corpus/artifact 路径；本地 GCC/ASan/UBSan smoke 通过，libFuzzer coverage 证据待远端 Clang CI |
| T16 300-player Pair | external-required | 只交付准备件 `docs/runbooks/capacity-300-player-pair.md`；未执行、未声称完成；保持 external-required |
| T17 文档 lint | done | `check-docs.py` 当前全量通过；`check-workflows.py`、`check-actions-pins.py`、`test-evidence-gate.py`、`release-fault-injection.sh` 均在 CI workflow-lint job 中执行并本地通过；`actionlint 1.7.12` 通过 |
| T18 Runbooks | needs-remediation | `docs/runbooks/release.md`、`rollback.md`、`gateway-security.md` 已落地且 gateway-security 更新 R2-F12 TLS 管理合同；release 演练（真实 registry）待发布时执行 |

**验证总览（实现提交 `04088aa90fd6c12dfb597b9dccbce7585a87a93f`，2026-08-11）**：CTest 420/420（GCC Debug）；GCC ASan Debug 420/420、GCC UBSan Debug 420/420、GCC TSan focused subset 360/360；gateway fuzz smoke 在普通/ASan/UBSan 构建均通过（256 inputs、frames_received=1024、length_rejected=256、json_rejected=512、零 master/pending 残留）；isolated runner 完整模式 PASS（LPC 断言 + 四个唯一 loopback 端口）+ 端口隔离压力 5 串行 20 并发零冲突零残留；`lpc-modern-runtime-stress.sh smoke` 通过；连续两次全目标构建的第二次无 C++ 编译/链接；check-docs、check-workflows --self-test、check-actions-pins --self-test（14 action SHA + 2 digest）、test-evidence-gate、release 故障注入、`actionlint 1.7.12`、SBOM CycloneDX、容量证据 envelope（3 份）均通过。Clang/macOS/Windows、CodeQL、Docker 与 libFuzzer coverage 必须由该实现提交之上的最终 PR HEAD 远端 CI 复核；本段本地结果不得外推为跨平台或发布演练完成。

**第二轮整改闭环（R2-F01..F13）**：`docs/pr-36-second-round-merge-audit-and-remediation-plan-2026-08-10.md` 的 13 项发现已全部落地并有本地证据；剩余跨平台/发布/registry 演练依赖远端环境，按 §11 门禁清单在 Phase 4 复核。
