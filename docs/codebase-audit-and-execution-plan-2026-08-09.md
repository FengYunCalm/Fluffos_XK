...
| 工作包 | 状态 | 交付物/证据 |
| --- | --- | --- |
| T01 sys_reload_tls 边界 | done | 边界修复（T01）＋ R2-F12 管理合同：主线程强制、master 授权 hook（`valid_sys_reload_tls`）、固定检查顺序（线程→授权→索引→TLS 类型）、失败原子性（旧 SSL_CTX 保持）。证据：`DriverTest.TestSysReloadTlsPermissionAndThreadMatrix` 覆盖 main+authorized/unauthorized、websocket/non-TLS、极值索引、worker+authorized 拒绝、reload 失败原子性；`testsuite/single/tests/efuns/sys_reload_tls.c`；`SECURITY.md` 与 `docs/runbooks/gateway-security.md` 已更新。本地 `lpc_tests` 416/416 通过 |
| T02 Release DAG 重排 | needs-remediation | R2-F08：target SHA 精确校验与直接依赖、JSON required-check 全集（18 项）、单次构建 OCI archive + skopeo digest + Trivy 阻断 + 同 digest 推广（远端 inspect 复核）、verify-release-inputs 只读 job、最小权限（仅 publish-draft/promote-image 在 protected environment 有写权限）、全局 release concurrency、dry_run 模式与故障注入（`tools/release/release-fault-injection.sh` 全过）。actionlint 全部 workflow 通过。剩余：真实 registry 受保护演练需发布时执行 |
...
| T05 Sanitizer 矩阵 | needs-remediation | 矩阵结构与 TSan 子集断言已就绪（check_name 唯一化，R2-F01）；本地无 Clang，ASan/UBSan/TSan 全平台结果待当前 HEAD 远端 CI run |
| T06 CMake 生成污染 | needs-remediation | 改动已落地；最终 HEAD 二次构建零重编译与源码树无生成物复核待 Phase 4 执行 |
...
| T10 OwnerExecutor 异常 | needs-remediation | 异常终态化/RAII 已落地；TSan subset 结果待远端 CI |
| T11 ObjectHandle 引用 | done | R2-F06：主线程 admission 全入口强制（handle 与 raw 路径统一拒绝 `main_thread_admission_required`，release 构建生效）、`VMObjectRefGuard` 拆为 `release_ref`/`detach`、worker ref-mutation 探针只计实际 off-main 变更。证据：`TestVmOwnerWorkerNestedObjectTargetSubmissionRejected`（worker 内 owner_call_async/owner_async/vm_owner_lpc_task/ordinary LPC 全拒绝且 mutation=0）、`TestVmOwnerWorkerNeverMutatesObjectRefcounts`；416/416 通过。ASan/TSan 复验待远端 |
| T12 Future 回收 | done | R2-F05：生命周期槽位预留（admission 成功→终态可查询不变量）、quota 拒绝保留 failed 墓碑（poll/state/take 可见直至 take/TTL）、insert 拆分 admit_pending/restore_terminal_checked、frozen 计量覆盖 mapping key/value 与 MSTR_SIZE、深度/节点/饱和上限、all-terminal 与 reapable 双时间索引、全入口共享记账。证据：`OwnerFutureStoreTest` 15 项含并发压力、`TestFutureFrozenMappingKeyAndValueBytesAreCounted`、容量 envelope（`docs/reports/capacity-evidence-2026-08-11/`） |
...
| T15 边界与 fuzz | needs-remediation | R2-F02/F04：fuzz 目标拆分（默认构建不含无 main 目标、非 Clang 开开关 configure 失败）、fixture 生命周期（fd 重解析、空输入安全、init 失败 abort）、smoke 硬断言（接受/拒绝/零泄漏）、corpus 提交、CI fuzz 有界运行；libFuzzer coverage 证据待远端 CI（本地无 Clang） |
| T16 300-player Pair | external-required | 只交付准备件 `docs/runbooks/capacity-300-player-pair.md`；未执行、未声称完成；保持 external-required |
| T17 文档 lint | done | `check-docs.py` 963/963 通过（本轮多次验证）；`check-workflows.py`、`check-actions-pins.py`、`test-evidence-gate.py`、`release-fault-injection.sh` 均在 CI workflow-lint job 中执行并本地通过 |
...
**验证总览（本分支当前 checkout，2026-08-11）**：CTest 416/416（Debug，含 R2-F05/F06/F10/F12 新增矩阵）、gateway fuzz smoke 硬断言通过（frames_received=1024、length_rejected=256、json_rejected=256、零泄漏）、isolated runner 完整模式 PASS（LPC 断言 + 四个唯一 loopback 端口）+ 端口隔离压力 5 串行 20 并发零冲突零残留、check-docs 963/963 OK、check-workflows --self-test OK、check-actions-pins --self-test OK（14 action SHA + 2 digest）、test-evidence-gate 全部负例拒绝、release 故障注入全过、actionlint 全部 workflow 通过、SBOM CycloneDX 校验 OK、容量证据 envelope（3 份）门禁全过。ASan/UBSan/TSan 与 Clang/macOS/Windows 矩阵、CodeQL、Docker、libFuzzer coverage 待当前 HEAD 远端 CI 验证（本地无 Clang）。
