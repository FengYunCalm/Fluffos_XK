// Regression test for upstream issue #1295 (fix ec9b6a4a), ported to .c.
...
//    stack held the last reference to a destructed object, the object -- and
//    obname with it -- was deallocated and the append read freed memory.
//    destruct2() cannot be forced from a synchronous testsuite run, so that
//    half is pinned behaviorally: the append must happen before the
